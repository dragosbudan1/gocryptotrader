...
func (l *LakeBTC) SendAuthenticatedHTTPRequest(method, params string, result interface{}) (err error) {
	...
	req := fmt.Sprintf("tonce=%s&accesskey=%s&requestmethod=post&id=1&method=%s&params=%s", l.Nonce.String(), l.API.Credentials.Key, method, params)
	hmac := common.GetHMAC(common.HashSHA1, []byte(req), []byte(l.API.Credentials.Secret))
	...
	headers["Authorization"] = "Basic " + common.Base64Encode([]byte(l.API.Credentials.Key+":"+common.HexEncodeToString(hmac)))
	headers["Content-Type"] = "application/json-rpc"

	return l.SendPayload("POST", l.API.Endpoints.URL, headers, strings.NewReader(string(data)), result, true, l.Verbose)
}
