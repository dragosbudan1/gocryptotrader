...
func (l *LocalBitcoins) SendAuthenticatedHTTPRequest(method, path string, params url.Values, result interface{}) (err error) {
	...
	message := l.Nonce.String() + l.API.Credentials.Key + path + encoded
	hmac := common.GetHMAC(common.HashSHA256, []byte(message), []byte(l.API.Credentials.Secret))
	...
	if l.Verbose {
		log.Printf("Sending POST request to `%s`, path: `%s`, params: `%s`.", l.API.Endpoints.URL, path, encoded)
	}
	...
	return l.SendPayload(method, l.API.Endpoints.URL+path, headers, strings.NewReader(encoded), result, true, l.Verbose)
}
